...
export class CraftingModelService extends Disposable implements ICraftingModelService {
	private _taskSingerGetModelByPurpose = new TaskSingler<CraftingModel | null>();
	private _taskSingerGetModels = new TaskSingler<CraftingModel[]>();
	...
	async getModelByPurpose(purpose: CraftingModelPurpose, ignoreCache?: boolean): Promise<CraftingModel | null> {
		...
		const m = await this._taskSingerGetModelByPurpose.getOrCreate(`getModelByPurpose:${purpose}`, () => this.fetchModelByPurpose(purpose));
		if (m) {
			// Replace with the new map to trigger change event.
			const newMap = new Map(this._purposeModelMap);
			this._purposeModelMap = newMap.set(purpose, m);
	...
	async getModels(): Promise<CraftingModel[]> {
		...
		const models = await this._taskSingerGetModels.getOrCreate('getModels', () => CraftingModelService.fetchModels());
		this._models = models;
	...
	private async fetchModelByPurpose(purpose: CraftingModelPurpose): Promise<CraftingModel | null> {
		try {
			const resp = await this.fetcherService.fetch(`http://${craftingLLMAPIHost}/models/${purpose}?extra=y`, { callSite: '', method: 'GET' });
			if (resp.status === 404) {
